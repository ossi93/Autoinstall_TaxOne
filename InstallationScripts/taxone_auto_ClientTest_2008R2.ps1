...
$bt_DB.add_Click({
	...
	$newDB = $tb_database.get_Text()
		if ($newDB -like "") {
            ...
            sleep -Seconds 3
			Add-Content -Encoding Unknown -Path "$destPathDB\$newDBfile" -Value "CREATE DATABASE [$newDB] ON  PRIMARY 
...
## Add "Save & Close" Event
    $bt_svcSave.add_Click({
        ...
        Write-Log "$newDBFile has been created at $destPathDB" 
	    Add-Content -Encoding Unknown -Path "$destPathDB\$accName.sql" -Value "USE [master]
GO
CREATE LOGIN [DE\$accname] FROM WINDOWS WITH DEFAULT_DATABASE=[master]
GO
USE [$newDB]
GO
CREATE USER [de\de-svccoetax_service] FOR LOGIN [de\$accName]
GO
USE [$newDB]
GO
